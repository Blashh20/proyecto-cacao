...
export function LoginModal({ isOpen, onClose, onSwitchToRegister, onLogin }: LoginModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (role === "admin" && email === "admin@makakaw.co" && password === "admin123") {
      onLogin("admin")
      onClose()
    } else if (role === "user" && email && password.length >= 6) {
      onLogin("user")
      onClose()
    } else {
      setError(role === "admin" ? "Credenciales de administrador incorrectas" : "Email o contrasena incorrectos")
    }
    setIsLoading(false)
  }
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {role === "admin" && (
              <div className="rounded-xl border border-forest/30 bg-forest/10 px-4 py-3 text-sm text-forest-light">
                <strong>Demo Admin:</strong> admin@makakaw.co / admin123
              </div>
            )}
