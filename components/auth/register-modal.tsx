...
export function RegisterModal({ isOpen, onClose, onSwitchToLogin, onRegister }: RegisterModalProps) {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    const newValue = name === "numero_identificacion" ? value.replace(/\D/g, "") : value
      ...
      <div className="relative w-full max-w-md overflow-hidden rounded-2xl border border-border bg-card shadow-2xl animate-in fade-in zoom-in-95 duration-300">
        ...
        <div className="p-8">
          ...
          <div className="mb-6 text-center">
            ...
            <h2 className="text-2xl font-serif font-bold text-foreground">Crear Cuenta</h2>
            <p className="mt-2 text-muted-foreground">Unete a la comunidad Makakaw</p>
          </div>

          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="mb-2 block text-sm font-medium text-foreground">Tipo de Identificacion</label>
              <select
            ...
            <div>
              <label className="mb-2 block text-sm font-medium text-foreground">Numero de Identificacion</label>
              <input
                type="tel"
            ...
            <div>
              ...
              <div className="mt-2 space-y-1">
                {passwordRequirements.map((req, index) => (
                  <div
                    key={index}
                    className={cn("flex items-center gap-2 text-xs transition-colors", req.met ? "text-forest" : "text-muted-foreground")}
                  >
                    <div
                      className={cn(
            ...
            <div>
              <label className="mb-2 block text-sm font-medium text-foreground">Confirmar Contrasena</label>
              <div className="relative">
                <input
            ...
            <div className="flex items-start gap-3">
              <button
                ...
                className={cn(
                  ...
                  acceptTerms ? "border-forest bg-forest" : "border-border hover:border-forest"
                )}
              >
                {acceptTerms && <Check size={12} className="text-white" />}
              </button>
              <span className="text-sm text-muted-foreground">
                Acepto los <a href="#" className="text-forest hover:underline">terminos y condiciones</a> y la{" "}
                <a href="#" className="text-forest hover:underline">politica de privacidad</a>
              </span>
            </div>
          ...
          <div className="mt-6 text-center">
            <p className="text-muted-foreground">
              ...
              <button onClick={onSwitchToLogin} className="font-semibold text-forest transition-colors hover:text-forest-light">
                Inicia sesion aqui
              </button>
            </p>
          </div>
